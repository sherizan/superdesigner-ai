...
- **Scaffold** (a PRD but no Figma target in `figma.md`) — propose the screens from the PRD and
  build a starting point: either Figma canvas frames (figma-console MCP) or a single-file HTML
  prototype. Still intent-first — every proposed screen traces to a PRD step; this is a first draft
  to react to, not a finished, pixel-perfect design. The engine selects the mode automatically by
  checking `figma.md` for a real Figma link/node.
...
**If the Figma Dev Mode MCP is connected, use it to analyze designs:**
...
**If the Figma MCP is not connected, do not stop.** Run the rest of the review against the
`figma.md` text (URLs, node IDs, screen notes) plus the PRD, research, analytics, and content
sources. Note in the review that live Figma inspection was skipped, and that screen-level node
IDs in comments are taken from `figma.md` rather than verified against the file. A review without
Figma inspection is still more rigorous than no review.
...
- `DESIGN.md` - Workspace-root design reference ("what good means"); the UX and content findings cite its rules
...
- `projects/<slug>/prototype/*` - Scaffold code prototype (HTML), when the code path is used
...
**Follow the template:** `.superdesigner/templates/design-review.template.html` — a self-contained,
